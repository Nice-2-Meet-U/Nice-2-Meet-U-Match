...
def get_user_pool(user_id: UUID):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to retrieve user pool: {str(e)}")
...
def get_user_matches(user_id: UUID):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to retrieve user matches: {str(e)}")
