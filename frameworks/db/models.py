...
class Pool(Base):
    ...
    location = Column(String(255), nullable=True, index=True, doc="Location/region of the pool")
    member_count = Column(Integer, nullable=False, default=0, doc="Number of members in the pool")
    created_at = Column(TIMESTAMP, server_default=func.now(), doc="Pool creation timestamp")
    ...
    matches = relationship(
        ...
        back_populates="pool", 
        cascade="all, delete-orphan",
        doc="Matches created in this pool"
    )
...
class PoolMember(Base):
    ...
    joined_at = Column(TIMESTAMP, server_default=func.now(), doc="When user joined the pool")
...
class Match(Base):
    ...
    pool_id = Column(
        ...
        nullable=False,
        index=True,
        doc="Pool where this match exists"
    )
    user1_id = Column(CHAR(36), nullable=False, index=True, doc="First user (normalized: user1_id < user2_id)")
    user2_id = Column(CHAR(36), nullable=False, index=True, doc="Second user (normalized: user1_id < user2_id)")
    status = Column(
        String(20),
        nullable=False,
    ...
    created_at = Column(TIMESTAMP, server_default=func.now(), doc="Match creation timestamp")
    updated_at = Column(
        TIMESTAMP, 
...
class MatchDecision(Base):
    ...
    decision = Column(
        String(10), 
        nullable=False,
        doc="Decision: accept or reject"
    )
    decided_at = Column(TIMESTAMP, server_default=func.now(), doc="When decision was made")

    # Relationships
    match = relationship("Match", back_populates="decisions")
    
    # Note: Composite primary key (match_id, user_id) already ensures uniqueness
